...
def print_karnaugh(table, func_idx, func_name):
    """Выводит карту Вейча-Карно для одной функции (func_idx: 4=y4..7=y1)."""
...
def bits_to_int(bits):
    result = 0
    for b in bits:
        result = result * 2 + (0 if b is None else b)
    return result
        ...
        for i in range(len(keys) - 1):
            for a in groups[keys[i]]:
                for b in groups[keys[i + 1]]:
                    if can_merge(a, b):
                        m = merge(a, b)
                        ones_in_m = sum(b for b in m if b is not None)
...
def minimize_all(table):
    ...
    for fi in range(4, 8):
        ...
        if len(cover_knf) <= len(cover_dnf):
            ...
            print(f"    ТДНФ: {fname} = {expr_dnf}  <- выбрана (короче)")
...
def count_gates(results):
    ...
    for n_in in or_gates:
        transistors += 2 * n_in
    print(f"\n  Ориентировочно транзисторов: ~{transistors // 2}")
