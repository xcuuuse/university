...
from app.ui.common import ask, fill_table, selected_row, warn
from app.ui.price_dialog import PriceDialog
from app.ui.product_dialog import ProductDialog

PRODUCT_HEADERS = ["Code", "Name", "pieces", "Category", "Manufacturer"]
PRICE_HEADERS = ["Date", "Price"]
...
class ProductsTab(QWidget):
    def __init__(self):
        ...
        add_price_button = QPushButton("Add price")
        delete_price_button = QPushButton("Delete price")
        ...
        price_box = QGroupBox("Price histories")
    ...
    def delete_product(self):
        product_id = self.current_product_id()
        if product_id is None:
            warn(self, "Select product")
            return
        product = repo.get_product(product_id)
        if not ask(self, f"Delete product «{product.product_name}»?"):
            return
        try:
    ...
    def delete_price(self):
        ...
        if row is None:
            warn(self, "Select price")
            return
        if not ask(self, "Delete price entry"):
            return
